    ...
    public class ProgramIcon
    {
        ...
        public bool IsValid
        {
            get
            {
                ...
                if (getInfo.Exists && (getInfo.Extension == ".ico" || getInfo.Extension == ".exe"))
                    return true;
                else
                    return false;
            }
        ...
        public void Create(string progID, string description, ProgramIcon defaultIcon, ExecApplication execApp, OpenWithList openWith)
        {
            if (progID != null)
            {
                if (defaultIcon.IsValid && execApp.IsValid)
                {
                    ...
                    {
                        FileInfo info = new FileInfo(defaultIcon.IconPath);
                        if (info.Extension == ".exe")
                            key.CreateSubKey("DefaultIcon").SetValue("", defaultIcon.IconPath + ", 1", RegistryValueKind.String);
                        else
